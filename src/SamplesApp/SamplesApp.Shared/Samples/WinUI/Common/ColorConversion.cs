...
namespace SamplesApp.Samples.WinUI.Common
{
	...
	internal class Hsv
	{
		...
		private Rgb HexToRgb(string input)
		{
			HexToRgba(input, out var rgbValue, out var _);
		...
		string RgbToHex(Rgb rgb)
		{
			...
			return hexValue.ToString("X6");
		}

		void HexToRgba(string input, out Rgb rgb, out double alpha)
		{
			...
			if (int.TryParse(input.Substring(1), out var hexValue) == false)
			{
				rgb = new Rgb(-1, -1, -1);
				alpha = -1;
				return;
		...
		string RgbaToHex(Rgb rgb, double alpha)
		{
